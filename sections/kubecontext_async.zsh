...
spaceship_async_job_load_kubecontext_async() {
  [[ $SPACESHIP_KUBECONTEXT_SHOW == false ]] && return

  (( $+commands[kubectl] )) || return

  async_job spaceship spaceship_async_job_kubecontext_async
}

spaceship_async_job_kubecontext_async() {
  local kube_context
  kube_context=$(kubectl config current-context 2>/dev/null) || return
...
spaceship_kubecontext_async() {
  [[ $SPACESHIP_KUBECONTEXT_SHOW == false ]] && return

  local kube_context="${SPACESHIP_ASYNC_RESULTS[spaceship_async_job_kubecontext]}"
